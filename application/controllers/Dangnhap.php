...
class Dangnhap extends CI_Controller {
    ...
    public function dangnhap(){
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            if($this->Mcustomer->customer_login($email, $password)!=FALSE){
                ...
                echo "<script>alert('Đăng nhập thành công! Chào mừng bạn đến với trang sản phẩm của chúng tôi !');</script>";
            }else{
            ...
            $this->data['title']='ShopTeam4 - Đăng nhập tài khoản';
            $this->data['view']='dangnhap';
            $this->load->view('frontend/layout',$this->data);
        }     
    }
    ...
    public function dangky(){
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            $this->email->from('ShopTeam4@gmail.com', 'ShopTeam4');
            $list = array('web.shopteam4@gmail.com');
            $this->email->to($list);
            $this->email->subject('Hệ thống ShopTeam4');
            $this->email->message('Người dùng '.$this->input->post('name').' vừa trở thành thành viên trên website của bạn !');
            if ($this->email->send()) {
                $this->data['msg']='Đăng ký thành công !';
                redirect('dang-nhap','refresh');
                echo "<script>alert('Đăng ký tài khoản thành công! Hãy đăng nhập và bắt đầu mua sắm !');</script>";
            } 
        }  
        $this->data['title']='ShopTeam4 - Đăng ký tài khoản';   
