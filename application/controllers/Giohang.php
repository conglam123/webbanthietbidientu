...
class Giohang extends CI_Controller {
	...
	public function index(){
        $this->data['title']='ShopTeam4 - Giỏ hàng của bạn';
    ...
    public function checkout(){
        if($this->session->userdata('session24hStore')){
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                $this->data['title']='ShopTeam4 - Đặt hàng';
    ...
    public function info_order(){
        ...
        if($this->form_validation->run() == TRUE){
            ...
            if ($result) {
                ...
                echo "<script>console.log( 'Da gui xong ' );</script>";
            }
        }else{
            $this->data['title']='ShopTeam4 - Thông tin đơn hàng';
    ...
    public function thankyou(){
        if($this->session->userdata('info-customer')||$this->session->userdata('session24hStore')){
            ...
            $this->email->set_newline("\r\n");
            $this->email->from('ShopTeam4@gmail.com', 'Shop Team4');
            $list = array($val['email']);//Email của khách hàng
            $this->email->to($list);
            $this->email->subject('Hệ thống ShopTeam4');
        ...
        $this->data['title']='ShopTeam4 - Kết quả đơn hàng';
        $this->data['view']='thankyou';
        $this->load->view('frontend/layout',$this->data);
        echo "<script>console.log( 'Da xong' );</script>";
    }
    ...
    public function district()
    {
        $html = array("arr1"=>'');
        $this->load->library('session');
        $html['arr1'].='<option value ="">--- Chọn quận huyện ---</option>';
        if($this->input->post('provinceid'))
        {
            ...
            foreach ($list as $row) 
            {
                $html['arr1'].='<option value="'.$row["maqh"].'">'.$row["name"].'</option>';
            }
        } 
